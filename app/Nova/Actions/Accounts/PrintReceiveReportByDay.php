...
class PrintReceiveReportByDay extends Action
{
    ...
    {
        return 'print-receive-report-by-day';
    }
    /**
    ...
    public function handle(ActionFields $fields, Collection $models)
    {
        ...
        $from  =  $decodedFilters->firstWhere('class', 'App\Nova\Filters\Lenses\ReceiveLensFromDate');
        $from_value = Arr::get($from, 'value');
        if ($from_value == '') {
            return Action::danger('เลือก วันที่เริ่มต้น ที่ต้องการที่เมนูกรองข้อมูลก่อน');
        }
        $to  =  $decodedFilters->firstWhere('class', 'App\Nova\Filters\Lenses\ReceiveLensToDate');
        $to_value = Arr::get($to, 'value');
        if ($to_value == '') {
            return Action::danger('เลือก วันที่สิ้นสุด ที่ต้องการที่เมนูกรองข้อมูลก่อน');
        }
        return Action::openInNewTab('/car/report_12/' . $from_value . '/' . $to_value);
    }
